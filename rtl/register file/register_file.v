module register_file #(
    ...
    always @(*) begin
        for (i = 0; i < NUM_READ_PORTS; i=i+1) begin
            ...
            read_data[(i+1)*DATA_WIDTH-1 -: DATA_WIDTH] = register[read_addr[(i+1)*ADDR_WIDTH-1 -: ADDR_WIDTH]];
        end
    end
    ...
    always @(posedge clk or posedge rst) begin
        ...
        if (rst) begin
            for (j = 0; j < DEPTH; j=j+1) begin
                register[j] <= {DATA_WIDTH{1'b0}};       //Reset register -> Read ports also gets reset
            end
        end
        ...
        //Synchronous write
            ...
            for (k = 0; k < NUM_WRITE_PORTS; k=k+1) begin
                ...
                //Port of higher value is written if multiple write ports are enabled for the same address
                if (write_en[k]) begin

                    //Write data for each port using write_addr 
                    register[write_addr[(k+1)*ADDR_WIDTH-1 -: ADDR_WIDTH]] <= write_data[(k+1)*DATA_WIDTH-1 -: DATA_WIDTH];
                end
            end
        end
    end
